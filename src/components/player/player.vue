...
export default {
  ...
  methods: {
    ...
    _getPosAndScale() {
      const targetWidth = 40 // 小播放器图标值
      const paddingLeft = 40 // 小播放器padding-left值
      const paddingBottom = 30 // 小播放器padding-bottom值
      const paddingTop = 80 // 大播放器padding-top 值
      const width = window.innerWidth * 0.8 // 大CD图片的宽度
      const scale = targetWidth / width // 初始缩放比例
      const x = -(window.innerWidth / 2 - paddingLeft) // 初始x位置
      const y = window.innerHeight - paddingTop - width / 2 - paddingBottom // 初始y位置
      return {
        x,
        y,
        scale
